...
function ComboboxTrigger({
  ...
  return (
    ...
    >
      {children}
      <ChevronDownIcon className="pointer-events-none size-3.5 text-foreground/60" />
    </ComboboxPrimitive.Trigger>
  );
}
...
function ComboboxEmpty({ className, ...props }: ComboboxPrimitive.Empty.Props) {
  return (
    <ComboboxPrimitive.Empty
      ...
      className={cn(
        "hidden w-full justify-center py-2 text-center text-xs/relaxed text-foreground/60 group-data-empty/combobox-content:flex",
        className,
      )}
      {...props}
    />
...
function ComboboxChip({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "flex h-[calc(--spacing(4.75))] w-fit items-center justify-center gap-1 rounded-[calc(var(--radius-sm)-2px)] bg-foreground/60/10 px-1.5 text-xs/relaxed font-medium whitespace-nowrap text-foreground has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0",
        className,
      )}
      {...props}
    >
